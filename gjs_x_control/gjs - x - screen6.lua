...
local SEQUENCER_CHANNEL_SLOT = 1090
local SCREEN6_ACTIVE_SLOT = 1091
                                    ...
                                    elseif balance.position == 6 then index = 9 + balance.step
                                        elseif balance.position == 7 then index = 13 + balance.step
                                            elseif balance.position == 8 then index = 18
                                                    ...
                                                    reaper.gmem_write(SCREEN6_ACTIVE_SLOT, 1)
                                                    local C = api.COLOR
                                                            ...
                                                            for pitch = 0, 127 do
                                                                ...
                                                                if state.sequencer_layout ~= "piano" then return end
                                                                    for _, pitch in ipairs(pitches or {}) do
                                                                        ...
                                                                        -- One-shot cleanup when screen 6 is entered from another screen.
                                                                        -- Do NOT disable audition here; just remove stale pitches before
                                                                        -- the current piano selection is allowed to audition again.
                                                                        if not screen6_was_active then
                                                                            clear_audition_notes()
                                                                            screen6_was_active = true
                                                                            end
                                                                            ...
                                                                            for pitch, selected in pairs(state.sequencer_chord) do
                                                                                if selected then pitches[#pitches + 1] = pitch end
                                                                                    ...
                                                                                    reaper.gmem_attach("GJS_X_BRIDGE")
                                                                                    local ch = math.floor(tonumber(reaper.gmem_read(SEQUENCER_CHANNEL_SLOT)) or 0)
                                                                                    return math.max(0, math.min(15, ch))
                                                                                    end

                                                                                    local last_applied_sequencer_channel = nil

                                                                                    local function apply_sequencer_channel_to_current_item()
                                                                                    local channel = sequencer_midi_channel()
                                                                                    if last_applied_sequencer_channel == channel then return end

                                                                                        local context = sequencer.get_context()
                                                                                        local take = context and context.take
                                                                                        if not take then
                                                                                            last_applied_sequencer_channel = channel
                                                                                            return
                                                                                            end
                                                                                            ...
                                                                                            for note_index = 0, note_count - 1 do
                                                                                                ...
                                                                                                if ok and chan ~= channel then
                                                                                                    ...
                                                                                                    apply_sequencer_channel_to_current_item()
                                                                                                                ...
                                                                                                                if generation ~= display_generation then return end
                                                                                                                    if state.sequencer_layout == "midi_edit" then
                                                                                                                        ...
                                                                                                                        if api.get_current_screen and api.get_current_screen() ~= 6 then
                                                                                                                            -- Leaving the sequencer: first clear piano audition state.
                                                                                                                            -- performance.jsfx sees the zeroed pitches and emits the required
                                                                                                                            -- note-offs before screen6 is marked inactive.
                                                                                                                            set_audition_enabled(false)
                                                                                                                            reaper.gmem_attach("GJS_X_BRIDGE")
                                                                                                                            reaper.gmem_write(SCREEN6_ACTIVE_SLOT, 0)
                                                                                                                            screen6_was_active = false
                                                                                                                            sequencer.disable_display(1)
                                                                                                                            return
                                                                                                                            end
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    if result ~= "not_found" then found_existing = true end
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        if success and not found_existing then
                                                                                                                                                                                                                                                            for _, pitch in ipairs(pitches) do
                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                channel = sequencer_midi_channel(),
                                                                                                                                                                                                                                                                gate = gate,
                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                on_release = function()
                                                                                                                                                                                                                                                                -- Let the SysEx Bridge sequencer renderer restore its normal red/velocity state
