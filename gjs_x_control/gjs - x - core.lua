...
local function set_jsfx_loop_overview_active(active)
...
local function set_track_and_region(track, region)
    -- This function is reserved for screen 0.
    local state = get_screen_state(0)
    state.radio["tracks"] = 10 + track
    state.radio["regions"] = 60 + region

    set_scene_pattern(track, region)
end
...
local function update_loop_overview()
    if LP.jsfx_loop_overview_active
       or not LP.screen0_main_active
...
local function draw_current_screen()
    ...
    LP.jsfx_loop_overview_active = false
...
API.set_jsfx_loop_overview_active = set_jsfx_loop_overview_active
